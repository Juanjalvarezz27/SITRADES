...
export default function MuestraInactivaCard({
  ...
  // Determinamos el estado y el propósito
    ...
    <div className={`bg-white border border-slate-100 rounded-[2.5rem] p-6 shadow-sm hover:shadow-xl transition-all duration-500 flex flex-col h-full group relative overflow-hidden ${esAnulada ? 'hover:shadow-red-500/10' : 'hover:shadow-blue-500/10'}`}>
      ...
      <div className="flex items-start justify-between gap-4 mb-5 relative z-10 pr-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            {/* CONTENEDOR DE BADGES ORDENADO */}
            <div className="flex flex-wrap items-center gap-1.5 mb-2">
              {/* Etiqueta dinámica de Estado */}
              <span className={`inline-flex items-center gap-1 px-2 py-0.5 border rounded text-[9px] font-black uppercase tracking-wider ${esAnulada ? 'bg-red-50 text-red-600 border-red-200' : 'bg-blue-50 text-brand-primary border-blue-200'}`}>
                {esAnulada ? <AlertOctagon size={10} strokeWidth={3} /> : <Trash2 size={10} strokeWidth={3} />}
                {esAnulada ? 'Anulada' : 'Archivo Final'}
              </span>

              {/* Etiqueta dinámica de Propósito */}
              <span className={`inline-flex items-center gap-1 px-2 py-0.5 border rounded text-[9px] font-black uppercase tracking-wider ${
                esAnalisis ? 'bg-indigo-50 text-indigo-600 border-indigo-200' : 'bg-slate-50 text-slate-500 border-slate-200'
              }`}>
                <Target size={10} strokeWidth={3} />
                {esAnalisis ? 'Análisis' : 'Contramuestra'}
              </span>
            </div>
