...
interface Props {
  ...
  //  Nueva propiedad para manejar el clic en eliminar
  onDeleteClick: () => void; 
}

export default function GestionarUsuarioModal({ isOpen, onClose, usuario, onDeleteClick }: Props) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 transition-opacity">
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden transform scale-100 transition-transform duration-150">
        ...
        <div className="flex items-center justify-between p-5 border-b border-slate-100 bg-slate-50/50">
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              <h3 className="font-bold text-slate-800 text-[15px] truncate">{usuario.nombre}</h3>
              <p className="text-[12px] font-medium text-slate-500 truncate">{usuario.rol.nombre}</p>
            </div>
          </div>
        ...
        <div className="p-6 flex flex-col gap-3">
          <button className="flex items-center gap-4 p-4 border border-slate-100 rounded-2xl hover:border-brand-primary hover:bg-brand-primary/5 hover:shadow-sm transition-all group text-left">
            <div className="p-3 bg-brand-primary/10 text-brand-primary rounded-xl group-hover:scale-110 transition-transform shrink-0">
              <Edit2 size={20} />
            </div>
            <div>
              <p className="font-bold text-slate-800 text-[14px]">Editar información</p>
              <p className="text-[12px] font-medium text-slate-500">Modificar rol, área o datos de acceso</p>
            </div>
          </button>

          {/*  Conectamos el botón al evento onDeleteClick */}
          <button 
          ...
          >
            ...
            <div>
              <p className="font-bold text-slate-800 text-[14px]">Eliminar usuario</p>
              <p className="text-[12px] font-medium text-slate-500">Revocar acceso permanentemente del sistema</p>
            </div>
          </button>
        </div>
      </div>
